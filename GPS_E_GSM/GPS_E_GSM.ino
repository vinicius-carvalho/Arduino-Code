...
void loop(){
       ...
       if(gpsSerial.available()){ // Verifica se existe dado doGPS
...
//Mostra a informação no Serial monitor
   lat=String(slat == TinyGPS::GPS_INVALID_F_ANGLE ? 0.0 : slat, 6);
   lon=String(slon == TinyGPS::GPS_INVALID_F_ANGLE ? 0.0 : slon, 6);
   delay(2000);
